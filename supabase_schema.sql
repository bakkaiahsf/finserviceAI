...
-- UK Business Intelligence Platform with Row Level Security (RLS)
...
-- Create updated users table (extending existing)
...
CREATE TABLE plans (
  id SERIAL PRIMARY KEY,
  slug VARCHAR(50) NOT NULL UNIQUE, -- free, pro, proplus, expert
  name VARCHAR(100) NOT NULL,
  monthly_price_cents INTEGER NOT NULL DEFAULT 0,
  yearly_price_cents INTEGER NOT NULL DEFAULT 0,
  entitlements JSONB NOT NULL, -- { maxSearches: number, maxSeats: number, features: string[] }
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Teams/Organizations table (existing, updated for Nexus AI)
...
CREATE TABLE companies (
  ...
  sic_codes JSONB, -- array of SIC codes
  last_ch_sync TIMESTAMP WITH TIME ZONE,
  companies_house_data JSONB, -- raw API response
  ai_summary TEXT,
  ai_risk_score NUMERIC(5,2), -- 0-100
  ai_insights JSONB, -- confidence, caveats, prompt_version, etc
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE company_officers (
  ...
  resigned_on DATE,
  address JSONB,
  date_of_birth JSONB, -- partial DOB from Companies House
  nationality VARCHAR(100),
  nature_of_control JSONB, -- array for PSCs
...
CREATE TABLE ai_processing_jobs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  job_type VARCHAR(50) NOT NULL, -- 'company_summary', 'risk_assessment', etc
  entity_id UUID NOT NULL, -- references companies.id usually
  entity_type VARCHAR(50) NOT NULL, -- 'company', 'officer', etc
  status VARCHAR(20) NOT NULL DEFAULT 'pending', -- pending, processing, completed, failed
  input_data JSONB, -- input parameters for AI
  ai_response JSONB, -- AI model response
  provider_usage JSONB, -- tokens used, cost, model version, etc
...
CREATE POLICY "Team members can view their teams" ON teams FOR SELECT USING (
  EXISTS (SELECT 1 FROM team_members WHERE team_members.team_id = teams.id AND team_members.user_id = auth.uid()::integer)
);
CREATE POLICY "Admins and team owners can manage teams" ON teams FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role = 'admin') OR
  EXISTS (SELECT 1 FROM team_members WHERE team_members.team_id = teams.id AND team_members.user_id = auth.uid()::integer AND team_members.role IN ('owner', 'admin'))
);

-- RLS Policies for Companies table (core business data)
CREATE POLICY "Authenticated users can view companies" ON companies FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Analysts and above can modify companies" ON companies FOR INSERT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role IN ('admin', 'compliance_officer', 'analyst'))
);
CREATE POLICY "Analysts and above can update companies" ON companies FOR UPDATE USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role IN ('admin', 'compliance_officer', 'analyst'))
);

-- RLS Policies for Company Officers
CREATE POLICY "Authenticated users can view officers" ON company_officers FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Analysts and above can modify officers" ON company_officers FOR INSERT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role IN ('admin', 'compliance_officer', 'analyst'))
);
CREATE POLICY "Analysts and above can update officers" ON company_officers FOR UPDATE USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role IN ('admin', 'compliance_officer', 'analyst'))
);

-- RLS Policies for AI Processing Jobs
CREATE POLICY "Users can view own AI jobs" ON ai_processing_jobs FOR SELECT USING (created_by = auth.uid()::integer);
CREATE POLICY "Users can create AI jobs" ON ai_processing_jobs FOR INSERT USING (created_by = auth.uid()::integer);
CREATE POLICY "Admins can view all AI jobs" ON ai_processing_jobs FOR SELECT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role = 'admin')
);

-- RLS Policies for Search History
CREATE POLICY "Users can view own search history" ON search_history FOR SELECT USING (user_id = auth.uid()::integer);
CREATE POLICY "Users can create search history" ON search_history FOR INSERT USING (user_id = auth.uid()::integer);
CREATE POLICY "Team members can view team search history" ON search_history FOR SELECT USING (
  EXISTS (SELECT 1 FROM team_members WHERE team_members.team_id = search_history.team_id AND team_members.user_id = auth.uid()::integer)
);

-- RLS Policies for Quota Counters
CREATE POLICY "Users can view own quota" ON quota_counters FOR SELECT USING (user_id = auth.uid()::integer);
CREATE POLICY "System can manage quotas" ON quota_counters FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Team members can view team quota" ON quota_counters FOR SELECT USING (
  EXISTS (SELECT 1 FROM team_members WHERE team_members.team_id = quota_counters.team_id AND team_members.user_id = auth.uid()::integer)
);

-- RLS Policies for Audit Logs
CREATE POLICY "Users can view own audit logs" ON audit_logs FOR SELECT USING (user_id = auth.uid()::integer);
CREATE POLICY "Admins can view all audit logs" ON audit_logs FOR SELECT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid()::integer AND role = 'admin')
);
CREATE POLICY "System can create audit logs" ON audit_logs FOR INSERT USING (auth.role() = 'service_role');
